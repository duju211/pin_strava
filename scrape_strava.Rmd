...
```{r, code=df_manifest$command[df_manifest$name == "act_ids"], eval=FALSE}
```
...
```{r code=df_manifest$command[df_manifest$name == "df_meas_all"], eval=FALSE}
```
...
Turn the data into a wide format so that every activity is one row again:
...
Visualize the final data by displaying the geospation information in the data.
